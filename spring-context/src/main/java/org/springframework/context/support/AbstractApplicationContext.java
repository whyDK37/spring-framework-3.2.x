...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			...
			// 初始化 beanFactory ，进行xml文件读取。
			// 这一步完成复用 beanFactory 的配置文件读取和其它功能。
			ConfigurableListableBeanFactory beanFactory = obtainFreshBeanFactory();

			// Prepare the bean factory for use in this context.
			// 对 beanFactory 进行填充
			// 大家熟悉的 Qualifier , Autowired 就是在这一步完成的。
			...
			try {
				...
				// 注册拦截 bean 创建的 bean 处理器，这时只是注册，真正的调用是在 getBean 时。
				...
				// 在所有注册 bean 中查找 Listener bean ,注册到消息广播器中
				registerListeners();

				// Instantiate all remaining (non-lazy-init) singletons.
				// 初始化剩下的单例（非 lazy init）
				finishBeanFactoryInitialization(beanFactory);

				// Last step: publish corresponding event.
				// 完成刷新过程，通知生命周期处理器 LifecycleProcessor 刷新过程，同时发出 ContextRefreshedEvent 通知监听者
				finishRefresh();
			}
	...
	protected ConfigurableListableBeanFactory obtainFreshBeanFactory() {
		// 初始化 BeanFactory 并进行xml读取，并将得到的beanFactory记录在当前 context中。
		refreshBeanFactory();
	...
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		...
		// 设置 beanFactory的 classLoadder为当前 context 的 classLoader
		beanFactory.setBeanClassLoader(getClassLoader());
		// 设置 beanFactory 的表态式语言处理器，spring3增加了表达式语言的支持
		// 默认可以使用#{bean.XXX}的形式来调用相关属性值。
		beanFactory.setBeanExpressionResolver(new StandardBeanExpressionResolver());
		//增加一个默认的propertyEditor, 这个主要是对bean的属性等设置管理的一个工具
		...
		// 设置几个自动装配的特殊规则。
		// 个人理解这里注册的对象是为了暴露 spring 内部使用的对象，如beanFactory,ApplicationContext
	...
	protected void invokeBeanFactoryPostProcessors(ConfigurableListableBeanFactory beanFactory) {
		...
		// 对于配置中读取的 beanFactoryPostProcesor 的处理
		...
		// 最后，调用所有的 BeanFactoryPostProcessors
	...
	protected void registerBeanPostProcessors(ConfigurableListableBeanFactory beanFactory) {
		...
		// 当 spring 的配置中的后处理器还没有被注册就已经开始了 bean的初始化时
		...
		// 3 Finally, re-register all internal BeanPostProcessors.
